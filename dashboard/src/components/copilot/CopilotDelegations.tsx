...
export default function CopilotDelegations({ tasks, active, unavailable = false }: { tasks: readonly CopilotDelegateTask[]; active: boolean; unavailable?: boolean }) {
  ...
  return <section aria-label="Delegated tasks" className="space-y-2 rounded-lg border border-p-border p-3 text-sm">
    ...
    {unavailable ? <p>Delegated task status is unavailable.</p> : tasks.map(({ spawn, result: nativeResult, live, recovery }) => {
      const result = recovery ? recovery.recovery_state === 'settled' ? recovery : undefined : nativeResult
      return <article key={spawn.tool_id} className="rounded-md bg-p-accent-teal/5 p-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs">{recovery?.recovery_state === 'unverified' ? 'Interrupted or still running; cleanup not verified'
          : result ? ({ completed: 'Completed', failed: 'Failed', cancelled: 'Cancelled', limit_exceeded: 'Limit exceeded' }[result.status]) : active && live ? 'Running' : 'Incomplete: no result recorded'}</span>
