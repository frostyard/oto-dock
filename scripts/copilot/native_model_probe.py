#!/usr/bin/env python3
"""Two opt-in model turns proving native hook policy across cold SDK resume."""
...
MARKER = "OTO_NATIVE_MODEL_MARKER"
            ...
            async with asyncio.timeout(180):
                for generation in (1, 2):
                    ...
                    supervisor = CopilotSessionSupervisor(
                        pending_requests=frozenset, close_runtime=runtime.close, rpc_timeout=10, turn_timeout=60,
                    )
                    supervisors.append(supervisor)

                    async def decide(name, arguments):
                        counts[f"generation_{generation}_decisions"] += 1
                        allow = (generation == 1 and name == "Write"
                                 and arguments == {"file_path": "/workspace/model-proof.txt", "content": MARKER}
                                 and counts["generation_1_decisions"] == 1)
                    ...
                    policy = CopilotNativeToolPolicy(bridge, enabled_tools=frozenset({"create", "edit"}))
                    ...
                    if generation == 1:
                        ...
                        session = await policy.resume_session(client, session_id, **options)
                        report["cold_resume_with_reinstalled_gate"] = True
                    ...
                    prompt = (
                        f'Use create exactly once with path="/workspace/model-proof.txt" and file_text="{MARKER}". '
                        'Do not add a newline. Then reply DONE.' if generation == 1 else
                        f'Use edit exactly once with path="/workspace/model-proof.txt", old_str="{MARKER}", '
                        'and new_str="CHANGED". If denied, do not retry. Then reply DONE.'
                    )
                    result = [event async for event in supervisor.stream(prompt)]
                    assert counts[f"generation_{generation}_decisions"] >= 1
                    assert sum(event.type == "done" for event in result) == 1
                    assert (agent / "workspace/model-proof.txt").read_text() == MARKER
                    report.setdefault("turns", []).append({
                        "generation": generation, "done_count": 1, "fixture_unchanged": True,
                        "native_approve_all": True, "pending_host_requests": len(supervisor.requests.pending_ids),
                    })
                    await supervisor.close()
                    assert not runtime.alive and not runtime.forced_cleanup
                assert counts["allows"] == 1 and counts["denials"] >= 1
        finally:
...
def main():
    ...
    report = {"result": "failed", "sdk_version": "1.0.13", "runtime_version": "1.0.83",
              "live_turn_limit": 2, "sandboxed": True, "authority": "controlled fixture"}
